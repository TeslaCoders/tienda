...
const mostrarSpinner = () => {
   ...
   spinnerContenedor.innerHTML = `
    ...
    setTimeout (() => {
        spinner.remove();
        mostrarMensaje("Cargado de manera exitosa, pronto te llamaremos");
    }, 3000);
...
const mostrarMensaje = (mensaje, error=null) => {
    ...
    const alerta = document.createElement('p');  // p es la variable para crear parrafos
...
// (e) y la e tambien es un evento son los parameros que se pueden enviar 
export const validarFormulario = (e) =>{
    e.preventDefault();  // aun no se recargue deseo hacer mas cosas 
    ...
    if([nombre, telefono, correo, mensaje].includes("")){
        mostrarMensaje("Todos los campos son obligatorios perro", true);
        return;
    }
    ...
    Swal.fire({
        title: 'EStas seguro desea guardar?',
        showDenyButton: true,
        ...
        if (result.isConfirmed) {
          ...
          //mostrarMensaje("Enviando la información a la base de datos perrito");
